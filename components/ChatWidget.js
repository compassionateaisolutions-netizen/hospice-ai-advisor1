...
export default function ChatWidget({ embedded = false }) {
  ...
  const actualMessage = messageText || input
    ...
    const normalizedMessage = hasMessage ? String(actualMessage).trim().toLowerCase() : ''
    const resetTriggers = ['another patient', 'new patient', 'different patient', 'new case', 'next patient', 'start a new patient']
    const shouldResetContext = resetTriggers.some(trigger => normalizedMessage.includes(trigger))

    let threadIdToUse = threadId
    let fileIdsToUse = Array.isArray(fileIds) ? [...fileIds] : []
    ...
    try {
      ...
      const response = await fetch('/api/chat-assistant', {
        ...
        body: JSON.stringify({ 
          ...
          files: actualFiles,
          threadId: threadIdToUse, // Pass current thread for conversation continuity
    ...
    <div>
      ...
      {(open || embedded) && (
        <div className={embedded 
          ...
          : "fixed right-10 bottom-6 w-[34rem] md:w-[46rem] bg-white border rounded-2xl shadow-xl flex flex-col overflow-hidden z-50"
        }>
          {!embedded && (
          ...
          <div ref={messageContainerRef} className={`p-5 overflow-y-auto ${embedded ? 'h-[32rem]' : 'h-[28rem]'}`} id="chat-window">
            {messages.map((m) => (
